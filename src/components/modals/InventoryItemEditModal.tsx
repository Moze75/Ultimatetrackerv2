...
interface WeaponMeta { damageDice: string; damageType: 'Tranchant' | 'Perforant' | 'Contondant'; properties: string; range: string; category?: WeaponCategory; weapon_bonus?: number | null; }
...
export function InventoryItemEditModal({
  ...
  const [wCategory, setWCategory] = React.useState<WeaponCategory>(existingMeta.weapon?.category || 'Armes courantes');
    const [wBonus, setWBonus] = React.useState<number | null>(existingMeta.weapon?.weapon_bonus ?? null);
          ...
          {type === 'weapon' && (
            <div className="space-y-3 border-t border-gray-700 pt-4">
              ...
              <div>
                ...
                />
                <p className="text-xs text-gray-500 mt-1">
                  Ce bonus s'ajoute automatiquement aux calculs d'attaque et de dégâts
                </p>
              </div>
            </div>
          )}
